...
# Agenda w/ Julien & Michel.

* References updated.
* Relative influence:
  * Correction made/methodology.
  * What if colsample by tree is used in relative influence ?
* Extension to classical BT.
* Next developments:
  * Extension to the whole Tweedie family.
  * Writing.
...
$$
\mathrm{Var}(\mathrm{E}(Y)) = \mathrm{E}(Y)^p \text{ , for some } p \in \{0\} \cup [1, + \infty[.
$$
...
**We emphasize that the current package is limited to the *(*Adaptive*)* Boosting Tree methods and is only available for the Poisson distributed response variable.**
...
* As already mentioned, the simulated response variable `Y` follows a Poisson distribution (i.e. `tweedie.power` equals to 1).
...
* We'll work in the context of Adaptive Boosting Tree and perform 300 iterations (=`n.iter`).
...
Finaly, we note that all the trees fitted during the algorithm are stored in the `BTIndivFits` list:
...
Regarding the currently developed method, we used the results furnished by `rpart.object`. In fact, each tree in the expansion is built thanks to this package and is stored in the `BT$BTIndivFits` list. Looking at each tree, one can then observe the following values (for the first tree as example):

```{r}
BT_algo$BTIndivFits[[1]]$frame
BT_algo$BTIndivFits[[1]]$frame[BT_algo$BTIndivFits[[1]]$frame$var != "<leaf>",]
BT_algo$BTIndivFits[[1]]$splits
```

Looking at the `rpart.object` help file, the following definition is given for the `frame` and `splits` matrix.
...
**`improve`, which is the improvement in deviance given by this split, or, for surrogates, the concordance of the surrogate with the primary,** 
*and `index`, the numeric split point.  The last column `adj` gives the adjusted concordance for surrogate splits. For a factor, the `index` column contains the row number of the csplit matrix.  For a continuous variable, the sign of `ncat` determines whether the subset `x < cutpoint` or `x > cutpoint` is sent to the left.*

The idea is therefore to summarize the `improve` values for each splitting node (excl. surrogates and competing ones) across all variables and trees in the expansion. 
In other words, for each built tree, we will summarize the improvement across variables. We'll then add it up with other iterations to obtain a matrix containing the variable improvements through the iterations.
This improvement in therefore considered as the relative influence.

Please note that this algorithm has been adapted from `rpart` itself and therefore cover special cases (e.g. need to rescale for `anova` method).

One can then present the function arguments:
...
Per default, all the built trees will be used.
...
In our first example, we decided to fit an Adaptive Boosting Tree approach. However, one can be interested in fitting a Boosting Tree one.
We note that the cross-validation is relevant for the later case while it might not be needed for the former one. In fact, by construction the Adaptive Boosting Tree
will naturally ends up to root node, which will not bring more information. This is then a natural stopping criterion.

Let's fit a classical BT with the same parameters. All the presentation already made can be transposed in this specific case as well.

```{r}
classical_BT_algo <- BT(formula = as.formula("Y_normalized ~ Age + Sport + Split + Gender"),
                        data = dataset,
                        tweedie.power = 1,
                        ABT = F,
                        n.iter = 300,
...
One of the main purpose of Boosting Tree is to keep a constant number of internal nodes (= `interaction.depth`). On the other hand, this value should decrease for the Adaptive approach.


```{r}
table(sapply(seq(1, 300), function(xx){nrow(classical_BT_algo$BTIndivFits[[xx]]$frame[classical_BT_algo$BTIndivFits[[xx]]$frame$var != "<leaf>",])}))
whichNot4 <- which(sapply(seq(1, 300), function(xx){nrow(classical_BT_algo$BTIndivFits[[xx]]$frame[classical_BT_algo$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) !=4)
temp <- (lapply(whichNot4, function(xx){unique(classical_BT_algo$BTIndivFits[[xx]]$frame$var)}))
temp ; table(unlist(temp))
table(sapply(seq(1, 300), function(xx){nrow(BT_algo$BTIndivFits[[xx]]$frame[BT_algo$BTIndivFits[[xx]]$frame$var != "<leaf>",])}))
```

Please note that if e.g. `colsample.bytree` has been set to 2, one could have faced the following situation which is totally explainable due to database construction.

```{r}
classical_BT_algo_temp <- BT(formula = as.formula("Y_normalized ~ Age + Sport + Split + Gender"),
                        data = dataset,
                        tweedie.power = 1,
                        ABT = F,
                        n.iter = 300,
...
table(sapply(seq(1, 300), function(xx){nrow(classical_BT_algo_temp$BTIndivFits[[xx]]$frame[classical_BT_algo_temp$BTIndivFits[[xx]]$frame$var != "<leaf>",])}))
whichNot4 <- which(sapply(seq(1, 300), function(xx){nrow(classical_BT_algo_temp$BTIndivFits[[xx]]$frame[classical_BT_algo_temp$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) !=4)
temp <- (lapply(whichNot4, function(xx){unique(classical_BT_algo_temp$BTIndivFits[[xx]]$frame$var)}))
temp[1:5] ; table(unlist(temp))
```

### Tree construction in Boosting Tree.

All the trees in the expansion are built thanks to `rpart` package. The default `tree.control` are set such to grow the biggest tree possible. 
However, as in other boosting package, it might happen that some of the built trees do not reach the expected `interaction.depth`.
As we fully rely on `rpart` (we do not have full expansion control), in some specific cases we emphasize that some of the grown trees might not be able to meet the above requirement (i.e. the original `rpart` grown tree contains less than `interaction.depth` internal nodes).

As the user might be aware of, it's not so obvious to build tree for a given `interaction.depth` only relying on the `rpart` package.
We then want to explain how the trees are built in such approach.

Let's built a `rpart` object as it's done during `BT` and analyze it.

```{r fig.height = 5, fig.width = 5, fig.align = "center"}
library(rpart)
tree <- rpart(Y ~ Gender+Age+Split+Sport+offset(log(ExpoR)),
              data = dataset,
              method = "poisson",
              control = rpart.control(cp = -Inf, maxdepth = 4, xval = 0, minsplit = 2, maxsurrogate = 0, maxcompete = 0))

frame_woLeaf <- tree$frame[tree$frame$var != "<leaf>",] ; frame_woLeaf

# par(xpd=TRUE)
# plot(tree, compress=TRUE, uniform = T, branch = 0.5)
# text(tree, minlength = 4, splits = T, use.n = T, digits = 2, cex=0.65)
```

Thanks to the `snip.rpart` function, one can snip the trees to a given specific size. This functions take the following two arguments:
...
Therefore by selecting the correct node indices, one can delete the undesired subtrees and then ending up with a tree containing the wished number of internal nodes.
To do this selection, one has to look into the `rpart$frame` and `rpart$splits``dataframes.

By construction, one can observe that the node number are attributed in a level-wised fashion, i.e. the root node will have index 1, the first left (resp. right) internal node will receive index 2 (resp. 3), the deep two nodes will receive index (from left to right) 4, 5, 6 and 7, ...

To keep only `interaction.depth` internal nodes one then needs to drop all nodes with index greater than `interaction.depth`. The tree returned following this approach will of course be grown level-wise. Please note that using such trick can also help to different trees (e.g. leaf-wise, all splits to right, ...).

To keep only `interaction.detph` internal nodes a specific splitting strategy is applied. This one is similar to the strategy used in `gbm3` and consists in:

* If `interaction.depth` is set to 1, it is sufficient to reduce the original tree to the root node and its children.
* If `interaction.depth` is greater than 1, the idea is to build the subtree with an iterative fashion.
  * Assume that we've built a subtree with `k < interaction.depth` splits.
  * We'll consider all the improvement for this subtree's children. That's looking at the best terminal node in terms of deviance improvement.
  * For the children that is optimal for the above criteria, one can then split it. The original child now becomes a parent node while two more terminal nodes are created.
  * The process is repeated until `interaction.depth` splits are defined or that no further splits can be found. In the latter case, one can then end up with tree smaller than expected.
  Please note that only positive improvements are considered during the process.

We emphasize that this `snip.rpart` code can also be used to snip subtrees visually (user can snip using its mouse). This was in fact the first idea behind this function.

One can then prune the tree. Please note that the predictions are coherent with the new pruning whereas the `variable.importance` field is not updated accordingly.

```{r fig.height = 5, fig.width = 5, fig.align = "center"}
ff <- tree$frame[tree$frame$var != "<leaf>",]
ff <- cbind(ff, tree$splits)
...
treeSnipped <- rpart::snip.rpart(tree, toss=setdiff(ff$node, nodeToKeep))

frame_woLeaf <- treeSnipped$frame[treeSnipped$frame$var != "<leaf>",] ; frame_woLeaf

#par(xpd=TRUE)
#plot(treeSnipped, compress=TRUE, uniform = T, branch = 0.5)
#text(treeSnipped, minlength = 4, splits = T, use.n = T, digits = 2, cex=0.9)
```
