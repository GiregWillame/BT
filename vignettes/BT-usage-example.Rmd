...
In the insurance modelling for example, the offset allows to take into account the time exposure-to-risk. It therefore help to align the asked premium with respect to the contract duration.
...
* the observed claim count $Y$ with offset $d$ (e.g. the time-exposure) or,
...
The basic idea behind this algorithm consists in building weak leaners to explain the remaining error, using all the past iterations. It differs from the Gradient Boosting Methods as we're here boosting the residuals rather than the pseudo-residuals, using the defined underlying distribution rather than a gaussian approach.
...
```
For this simulated used-case, it therefore seems that the usual boosting tree approach performs better.
