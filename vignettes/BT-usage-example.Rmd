...
* BT package presentation.
* Relative influence:
  * Method discussion.
  * What if colsample by tree used in relative influence.
* Next developments (to be prioritized):
...
* D. Hainaut, J. Trufin and M. Denuit (2019). "Effective Statistical Learning Methods for Actuaries", vol. 1, 2 and 3. *Springer Actuarial*.
...
```{r}
head(BT_algo$BTData$training.set, 5)
...
Looking at the `rpart.object` help file, the following definition is given for the `splits` matrix.
...
The idea is therefore to summarize the `improve` values across all variables and trees in the expansion. In other words, for each built tree, we will summarize the improvement across variables. We'll then add it up with other iterations to obtain a matrix containing the variable improvements through the iterations.
