...
tree <- rpart(Y ~ Gender+Age+Split+Sport+offset(log(ExpoR)),
              data = dataset,
              method = "poisson",
              control = rpart.control(cp = -Inf, maxdepth = 4, xval = 0, minsplit = 2))
```
...
To do this selection, one has to look into the `rpart$frame` dataframe.

```{r}
frame <- tree$frame
frame_woLeaf <- frame[frame$var != "<leaf>",] ; frame_woLeaf
```
...
One can then prune the tree. Please note that the prediction are coherent with the new pruning whereas the `variable.importance` field is not updated accordingly.

```{r}
vecIndex <- sort(as.numeric(rownames(frame_woLeaf)))
treeSnipped <- rpart::snip.rpart(tree, toss=vecIndex[seq(4+1, length(vecIndex))])
frame <- treeSnipped$frame
frame_woLeaf <- frame[frame$var != "<leaf>",] ; frame_woLeaf
```

To be precise, please note that we added an extra condition to handle cases where the initially built tree contains less than `interaction.depth` internal nodes. 
In this case, the original built tree is kept and consists in the biggest possible tree that one can built using `rpart` (this kind of case can happen as we observed in the example above - i.e. the variables don't allow to have enough splits or, more generally, the user expects a "too big" tree).
